...
const Menu = styled.div`
  ...
  .menuTextContainer {
    ...
    border-radius: 10px;
    transition: all 0.3s ease;
    background-color: ${({ isOpen }) => (isOpen ? "#1667f3" : "transparent")};
    &:hover {
      background-color: #1a69f0;
    }
    &.active {
      border: 1px solid black;
      box-shadow: 0px 8px 10px -5px #111;
      background-color: #1a69f0;
    }
  }
`;
    ...
    <div>
      <SidebarContainer visible={sidebarVisible}>
        <ToggleButton visible={sidebarVisible} onClick={handleToggleSidebar}>
          {sidebarVisible ? (
            <img src="/toggle-sidebar-icon.png" alt="Close Sidebar" />
          ) : (
            <img src="/toggle-sidebar-icon.png" alt="Open Sidebar" />
          )}
        </ToggleButton>
        <h1>SOAPIFY</h1>
        <MenuContainer>
