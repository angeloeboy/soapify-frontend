    ...
    getProducts().then((res) => {
      if (res.products) {
        ...
        products.map((product) => {
          ...
          if (
            product.quantity_in_stock > product.minimum_reorder_level &&
            product.quantity_in_stock < 2 * product.minimum_reorder_level
          ) {
              ...
              paginatedProducts.map((product, index) => (
                <TableRows key={product.product_id}>
                  <TableData $bold $withImage>
                    <Image
                      src={
                        product.image_link == "testimage"
                          ? "/sabon.png"
                          : "/api/" + product.image_link.replace(/\\/g, "/")
                      }
                      alt="My Image"
                      width="40"
                      height="40"
                    ...
                    {activeActionContainer === index && (
                      ...
                      >
                        ...
                        {product.isActive && (
                          <p
                            onClick={() =>
                              goToInventoryPageAndAddInventory(
                                product.product_id
                              )
                            }
                          >
                        ...
                        <p
                          onClick={() =>
                            activateProductFunc(product.product_id)
                          }
                        >
                          Reactivate
                        </p>
                        <p
                          onClick={() =>
                            deactivateProductFunc(product.product_id)
                          }
                        >
                          Deactivate
                        </p>
