		...
		<DeactivateModalContainer>
			<div className="inner">
				...
				<p className="modal-description">
					You are packing the product {type}, which currently does not have a corresponding boxed version. Confirming this will automatically create a boxed
...
const PackInventory = ({ setIsPackPopUpOpen, getInventoryFunc, selectedInventory }) => {
	...
	const convertPcsToBoxFunc = async (e) => {
		...
		//check if there is a box version of the product
		const boxVersion = products.find((product) => product.pcProductID == selectedInventory.Product.product_id && product.isBox);
		console.log(boxVersion);
		if (boxVersion === undefined) {
			setWarning(true);
			return;
		}
	...
	const convertPcsToBoxFuncConfirm = async (e) => {
		const res = await convertPcsToBox(selectedInventory, quantity);
	...
	return (
		<PopupOverlay>
			...
			{warning && <DeactivateModal type={selectedInventory.Product.product_name} close={setWarning} confirm={convertPcsToBoxFuncConfirm} />}
		</PopupOverlay>
	);
};
